...
def JT(T):
    return jump(T)*dI
J = JT(T)
# No jump in the last term due to the fact that T is projected values on
# each mesh, thus, by moving the top mesh, the value moves with the mesh,
# while the bottom value is fixed, and has a material derivative
dJdOmega = (tan_div(s("-"), n("-"))*jump(T))*dI+dot(s("-"), grad(T("+")))*dI
