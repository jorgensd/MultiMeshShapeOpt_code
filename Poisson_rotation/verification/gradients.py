...
# that lives on both meshes, where the top mesh is moved.
...
T.assign_part(1, project(x1[1], FunctionSpace(meshes[1], "CG", degree)))
...
dJdOmega_top = (div(s)*inner(grad(T),grad(T))*dX
            -2*inner(dot(grad(s),grad(T)), grad(T))*dX)
...
dJds += assemble_multimesh(dJdOmega_top).inner(s_top.vector())
print(dJds)
