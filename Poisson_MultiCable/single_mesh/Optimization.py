...
class MultiCableOptimization():
    ...
    def __init__(self, num_cables, J, dJ):      
        self.g_scale = 1 # Scaling of coefficient for gradient constraint
        self.outer_radius = outer_radius # Radius of background cable
        self.inner_radius = rubber_radius # Radius for each inner cable
        self.distance_from_outer = 0.079166667*outer_radius # Distance for each cable to outer boundary
        self.distance_from_internal = 0.098039216*rubber_radius # Distance between each internal cable
        self.num_cables = num_cables
        self.inner_radius *= numpy.ones(num_cables)
    ...
    def solve_init(self, eval_J, eval_dJ):
        ...
        self.nlp.int_option('max_iter', 20)
